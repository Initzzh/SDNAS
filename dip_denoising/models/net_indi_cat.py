...
class Unet(nn.Module):
    def __init__(self, indi, in_channels = 1, features = 48):
        ...
        for level in range(self.level_amount):
            ...
            if(level != self.level_amount-1):
                dsl_out_features = self.encoder_units[level+1].features
            else:
                dsl_out_features =self.middle_units[0].features
            

            self.dsl_blocks.append(
                DownBlock(self.encoder_units[level].features,dsl_out_features, self.encoder_units[level].downsample_type)
            )
            ...
            if(level != self.level_amount-1):
                usl_out_features = self.decoder_units[level+1].features
            else:
                usl_out_features = self.middle_units[self.middle_unit_amount-1].features
            self.usl_blocks.append(
                UpBlock(usl_out_features, self.decoder_units[level].features, self.decoder_units[level].upsample_type)
            )
        
        # middle
        for middle_index in range(self.middle_unit_amount):
            self.middle_conv_blocks.append(
                ConvBlock(self.middle_units[middle_index].block_amount, self.middle_units[middle_index].features, self.middle_units[middle_index].features, self.middle_units[middle_index].conv_type)
            )
    ...
    def forward(self, x):
        ...
        for level in range(self.level_amount-1, -1, -1): # 从3到0
            ...
            encoder_conv_out = encoder_conv_outputs[level]
