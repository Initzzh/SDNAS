...
#         net_input = get_noise(input_depth, INPUT, (img_np.shape[1], img_np.shape[2])).type(dtype).detach() # 随机产生噪声
#         print("[*] input_type : noise")

#     return net, net_input


def get_net(img_np, noise_np, args, indi):
...
def image_restorazation(file, args, indi):
    import time 
    ...
    img_np, noisy_np = load_image_pair(file, task_type, args)  # noisy_np 25的噪声图片
    if args.GT_noise:
    ...
    net, net_input = get_net(img_np, noisy_np, args, indi) # net_input # dip :net_input为随机产生的噪声图片
    net.train()
    ...
    # args.writer = SummaryWriter(log_dir="dip_denoising/runs/%s/%s" % (args.exp_tag, args.desc + image_name))
    for ep in range(args.epoch):
        optimizer.zero_grad()
        total_loss, out = cal_loss(net_input, noise_torch)
        with torch.no_grad():
            mse_loss = torch.nn.functional.mse_loss(out, img_torch).item()
            diff_loss = total_loss.item() - mse_loss # total_loss 与 mse 的diff
        ...
        psnr_noisy = calculate_psnr(noisy_clip_np, out) # noisy_np, out
        psnr_gt = calculate_psnr(img_np, out) # img_clean out
        ...
        if (psnr_noisy - psnr_noisy_last < -5) and (ep > 5) :
            ...
            if True: #(ep % 50 == 0) and
                if running_avg is None:
                    ...
                    running_avg = running_avg * running_avg_ratio + out * (1 - running_avg_ratio) #  每个epoch的out进行求avg 
            ...
            print('Iteration %05d    total loss: %f / MSE: %f / diff: %f   PSNR_noisy: %f   psnr_gt: %f PSNR_gt_sm: %f' % (
                    ep, total_loss.item(), mse_loss, diff_loss, psnr_noisy, psnr_gt, psnr_gt_running), end='\r')

            last_net = [x.detach().cpu() for x in net.parameters()]
            psnr_noisy_last=psnr_noisy
        total_loss.backward() # 通过out 与 degradation_image 更新网络权重
        optimizer.step()
        scheduler.step()
    ...
    if True:
        ...
        save_CHW_np(np_save_dir + "/%s.png" % (image_name), out) # 每次的out 结果图片
        save_CHW_np(np_save_dir + "/%s_avg.png" % (image_name), running_avg) # 平均out结果图片
        save_CHW_np(np_save_dir + "/%s_max.png" % (image_name), max_out) # 最好的结果图片
        save_CHW_np(np_save_dir + "/%s_max_avg.png" % (image_name), maxavg_out)
        ...
        stat["time_consume"] = time.time()-start_time # time
        log_file = open(np_save_dir + "/%s_log.txt" % (image_name), "w")
...
#     noisy_clip_np = np.clip(noisy_np, 0, 255)
#     # Step 2. make model and model input
#     net, net_input = get_net(img_np, noisy_np, args) # net_input # dip :net_input为随机产生的噪声图片
#     net.train()
...
def read_dataset_file_list(eval_data):
    dataset_dir = "./dip_denoising/testset/%s/" % eval_data
...
def inference(indi):
    ...
    for file in file_list:
        ...
        stat = image_restorazation(file, args, indi)
        stat_list.append(stat)
        # stat_list.append({"time_consume": time.time()-t1})

    data = pd.DataFrame(stat_list, index= [i.split("/")[-1] for i in file_list])
    # os.makedirs("./dip_denoising/csv/%s/%s/"  % (args.task_type, args.exp_tag), exist_ok=True)
    # data.to_csv("./dip_denoising/csv/%s/%s/%s.csv" % ( args.task_type, args.exp_tag ,args.dip_type+args.desc))
    print("experiment done")
    print(data)
    print("time_consume:", (time.time()-t1))
    # return stat_list[0]["max_psnr_avg"]
    return stat_list[0]["max_psnr"]
