...
class s2s_test_first(nn.Module):
    def __init__(self, indi, img_ch=3, output_ch=3, act_type="LeakyReLU", pad='reflection' ):
        super(s2s_test_first, self).__init__()
        enc_ch = [48, 48, 48, 48, 48]
        dec_ch = [96, 96, 96, 96, 96]  # fixed
        self.upsample = nn.Upsample(scale_factor=2)
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        ...
        self.first_conv = nn.Conv2d(in_channels=img_ch, out_channels=enc_ch[0],kernel_size=3,stride=1,padding=1)
        # self.dsl1  = OPS['convolution_d'](img_ch, 48)
        self.scale_1 = OPS["maxpool2d"](48,48)
        self.scale_2 = OPS["maxpool2d"](48,48)
        self.scale_3 = OPS["maxpool2d"](48,48)
        self.scale_4 = OPS["maxpool2d"](48,48)
    ...
    def forward(self, x):
        # encoding path
        x0 = self.first_conv(x)
        # scale1 = self.scale_1(x0)
        # scale2 = self.scale_2(scale1)
        # scale3 = self.scale_3(scale2)
        # scale4 = self.scale_4(scale3)
        # print(x0.shape)
...
class s2s_test_MIMO(nn.Module):
    def __init__(self, indi, img_ch=3, output_ch=3, act_type="LeakyReLU", pad='reflection' ):
        super(s2s_test_MIMO, self).__init__()
        enc_ch = [48, 48, 48, 48, 48]
        dec_ch = [96, 96, 96, 96, 96]  # fixed
        self.upsample = nn.Upsample(scale_factor=2)
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        # d5_scale = d5
        # for _ in range(4):
        #     d5_scale = self.upsample(d5_scale)
        ...
        # d4_scale = d4
        # for _ in range(3):
        #     d4_scale = self.upsample(d4_scale)
        ...
        # d3_scale = d3
        # for _ in range(2):
        #     d3_scale = self.upsample(d3_scale)
        ...
        # d2_scale = d2
        # for _ in range(1):
        #     d2_scale = self.upsample(d2_scale)
        ...
        # print(d2_scale.shape,d3_scale.shape, d4_scale.shape,d5_scale.shape)
...
class UnetScale(nn.Module):
    def __init__(self, indi, in_channels=3, features=48):
        ...
        self.scale = indi.scale
        self.scale_module = OPS["maxpool2d"](features,features)
        ...
        for level in range(self.level_amount):
            ...
            # dsl
            if level == 0:
                dsl_in_features = features# in_channels, features
    ...
    def forward(self, x):
        ...
        scale_x = [None for _  in range(self.level_amount)]
        scale_x[0] = x
        for scale_index in range(1,self.level_amount):
            scale_x[scale_index] = self.scale_module(scale_x[scale_index-1])
        ...
        # encoder
        for level in range(self.level_amount):
            if level!=0 and self.scale[level]==1 :
                x = scale_x[level] + x 
                # print("x_scale",x.shape)
...
def check_active2(nl_num, connect_gene):
    ...
    params: 
        ...
        connect_gene: 连接方式
    returns:
        nl_active, pre_index, out_index, 
        block node是否激活, 前驱节点, block输出节点 
    ...
    # 最后一个节点是否激活，当connecxt_gene>1,最后一个节点激活
...
# 十进制转二进制 int-->[0,1,1]
def dec2bin(block_num, connect_type):
    
    val = connect_type
    connect_gene = [0 for _ in range(block_num)] # 010,001效果一样
...
class ConvBlockConnect(nn.Module):
    def __init__(self, in_channels, out_channels, conv_type, connect_type, block_num=3):
        super(ConvBlockConnect, self).__init__()
        connect_gene = dec2bin(block_num, connect_type)
        self.nl_active, self.nl_pre_index, self.out_index = check_active2(block_num,connect_gene)
        self.block_num = block_num
        self.block = nn.ModuleList([None for _ in range(block_num)])
        for i in range(block_num):
            # 当前节点未激活
            if self.nl_active[i]:
                # nl_pre_index[i] == 0 当前节点的输入是上一层block的output
                if 0 in self.nl_pre_index[i]:
                    block_node = OPS[conv_type](in_channels, out_channels)
                else:
                    block_node = OPS[conv_type](out_channels, out_channels)
                self.block[i] = block_node
            # 当前节点未激活
            
        # self.outputs = [None for _ in range(len(self.nl_active) + 1)]

    def forward(self, x):
        outputs = [None for _ in range(self.block_num+1)]
        outputs[0] = x
        for i in range(1, self.block_num+1):
            if self.nl_active[i-1]:
                # node 的输入x
                # index: node 的编号， 1，2，3 节点， 0节点是上个block的输出
                for j, index in enumerate(self.nl_pre_index[i-1]):
                    if j == 0:
                        input_t = outputs[index]
                    else:
                        input_t = input_t + outputs[index]
                # 节点i在nn.ModuleList()中对应的索引为i-1
...
#             nn.Conv2d(144, 96,3), nn.BatchNorm2d(96), nn.ReLU() 
#         )
...
#         out2 = self.conv_block(x)
#         out = out1 + out2
#         return out
# torch.manual_seed(0)

# test_x = torch.randn(1,144,512,512)
# # net = basicBlock()
# net(test_x)

# conv = nn.Conv2d(144,96,3)
# conv2 = nn.Conv2d(144, 96,3)
# out1 = conv(test_x)
# out2 =conv(test_x)
# out = out1 + out2
...
# for i in range(8):
#     test_x = torch.randn(1,144,512,512)
#     # connect_gene = dec2bin(3,i)
#     # check_active2(3, connect_gene)
#     net = ConvBlockConnect(144,96,'conv_3x3_leakyReLU',i)
#     print("net",net)
#     out = net(test_x)
...
class UnetConnect(nn.Module):
    def __init__(self, indi, in_channels=3, features=48):
        ...
        for level in range(self.level_amount):
            ...
            # print("level",level)
            self.dsl_blocks[level] = DownBlock(dsl_in_features, self.encoder_units[level].features , self.encoder_units[level].downsample_type)
            self.encoder_conv_blocks[level] = ConvBlockConnect(self.encoder_units[level].features,self.encoder_units[level].features, self.encoder_units[level].conv_type, self.encoder_units[level].connect_type)
            # self.encoder_conv_blocks[level] = ConvBlock(self.encoder_units[level].block_amount, self.encoder_units[level].features, self.encoder_units[level].features, self.encoder_units[level].conv_type)
        ...
        for level in range(self.level_amount-1, -1, -1):
            ...
            self.decoder_conv_blocks[level] = ConvBlockConnect(conv_in_features, self.decoder_units[level].features, self.decoder_units[level].conv_type, self.decoder_units[level].connect_type)
            # self.decoder_conv_blocks[level] = ConvBlock(self.decoder_units[level].block_amount, conv_in_features, self.decoder_units[level].features, self.decoder_units[level].conv_type)
            self.usl_blocks[level] = UpBlock(self.decoder_units[level].features, self.decoder_units[level].features, self.decoder_units[level].upsample_type)
