...
OPS = {
    ...
    'dil_conv_3x3_soft':lambda C_in, C_out: DilConv_SP(C_in, C_out, kernel_size = 3, stride =1, dilation = 2),

    'conv_3x3_leakyReLU':lambda C_in, C_out: conv_block_sp(C_in, C_out, kernel_size=3, act_fun="soft"), # "ReLU"
    'conv_5x5_leakyReLU': lambda C_in, C_out: conv_block_sp(C_in, C_out, kernel_size=5, act_fun="soft" ), # "ReLU"
    'conv_7x7_leakyReLU': lambda C_in, C_out: conv_block_sp(C_in, C_out, kernel_size=7, act_fun="soft"),
    'conv_block_last': lambda C_in, C_out: conv_block_last(C_in, C_out, act_fun="soft"),
    # 'conv_block_last': lambda C_in, C_out: conv_block_last(C_in, C_out),
    # 'bn':lambda C_in: bn(C_in, mode='bn'),
    'bn':lambda C_in: nn.BatchNorm2d(C_in),

    # downsample
    'maxpool2d': lambda C_in, C_out: MaxPoolD(C_in, C_out, kernel_size=3, stride=2, padding=1),
    'maxpool2d_conv': lambda C_in, C_out:MaxPool(C_in, C_out, kernel_size=3, stride=2, padding=1),
    'convolution_d': lambda C_in, C_out: ConvDown(C_in, C_out, kernel_size=3, stride=2, act_fun="soft"),
    # 'convolution_d': lambda C_in, C_out: ConvDown(C_in, C_out, kernel_size=3, stride=2, padding=1),
...
#         self.conv1 = nn.Conv2d(C_in, 64, kernel_size=3, stride=1, padding=1 )
#         self.conv2 = nn.Conv2d(64, 32, kernel_size=3, stride =1, padding =1 )
#         self.conv3 = nn.Conv2d(32, C_out, kernel_size=3, stride=1, padding=1 )
#         self.conv = nn.Sequential(
#             self.conv1, nn.BatchNorm2d(64), nn.ReLU(),
#             self.conv2, nn.BatchNorm2d(32), nn.ReLU(),
#             self.conv3, nn.BatchNorm2d(C_out), nn.ReLU(),
#         )
...
class DilConv_SP(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding ,dilation, act_fun='soft'):
        super(DilConv_SP,self).__init__()
        to_pad = int((kernel_size - 1) / 2)
    
        self.op = nn.Sequential(
            nn.ReflectionPad2d(to_pad),
            nn.Conv2d(C_in, C_out, kernel_size=kernel_size, stride=stride, padding=to_pad,dilation=dilation,bias=True),
            nn.BatchNorm2d(C_out),
            act(act_fun)
        )
    def forward(self, x):
...
class MaxPool(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class MaxPoolD(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=2, padding=1):
        super(MaxPoolD, self).__init__()
...
#     def __init__(self, C_in, C_out, kernel_size=3, stride =2, padding =1):
#         super(ConvDown, self).__init__()
#         self.down_conv = nn.Conv2d(C_in, C_out, kernel_size=kernel_size, stride=stride, padding=padding)
#         self.bn = nn.BatchNorm2d(C_out)
#         self.relu = nn.ReLU()
#         self.op = nn.Sequential(self.down_conv, self.bn, self.relu)
...
# bilinear
class BilinearD(nn.Module):
    def __init__(self, stride, C_in, C_out):
        super(BilinearD, self).__init__()
        self.scale = stride
        self.op = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False)
...
class NearestD(nn.Module):
    def __init__(self,stride, C_in, C_out):
        super(NearestD, self).__init__()
        self.scale=stride
        self.op = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False)
...
class AreaD(nn.Module):
    def __init__(self, stride, C_in, C_out):
        super(AreaD, self).__init__()
        self.scale = stride
        self.op = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False)
    def forward(self, x):
        # return F.interpolate(x, scale_factor=self.scale, mode='area')
